...
const SSL_METHOD *dtls_bad_ver_client_method(void);
...
const SSL_CIPHER *ssl_get_cipher_by_char(SSL *ssl,
...
int ssl_cert_set_cert_store(CERT *c, X509_STORE *store, int chain,
                                   int ref);
...
const SSL_CERT_LOOKUP *ssl_cert_lookup_by_pkey(const EVP_PKEY *pk,
                                                      size_t *pidx);
...
int ssl_get_server_cert_serverinfo(SSL *s,
...
int ssl_fill_hello_random(SSL *s, int server, unsigned char *field,
...
unsigned int ssl_get_max_send_fragment(const SSL *ssl);
unsigned int ssl_get_split_send_fragment(const SSL *ssl);

const SSL_CIPHER *ssl3_get_cipher_by_id(uint32_t id);
const SSL_CIPHER *ssl3_get_cipher_by_std_name(const char *stdname);
const SSL_CIPHER *ssl3_get_cipher_by_char(const unsigned char *p);
int ssl3_put_cipher_by_char(const SSL_CIPHER *c, WPACKET *pkt,
                                   size_t *len);
...
size_t ssl3_final_finish_mac(SSL *s, const char *sender, size_t slen,
                                    unsigned char *p);
...
int ssl_get_min_max_version(const SSL *s, int *min_version,
...
int tls13_export_keying_material_early(SSL *s, unsigned char *out,
...
int tls1_check_group_id(SSL *s, uint16_t group_id, int check_own_curves);
uint16_t tls1_shared_group(SSL *s, int nmatch);
int tls1_set_groups(uint16_t **pext, size_t *pextlen,
                           int *curves, size_t ncurves);
int tls1_set_groups_list(uint16_t **pext, size_t *pextlen,
                                const char *str);
...
int ssl_security_cert_chain(SSL *s, STACK_OF(X509) *sk, X509 *ex,
                                   int vfy);
...
#  endif
int tls12_check_peer_sigalg(SSL *s, uint16_t, EVP_PKEY *pkey);
int ssl_set_client_disabled(SSL *s);
int ssl_cipher_disabled(const SSL *s, const SSL_CIPHER *c, int op, int echde);
 ...
 */
int ssl_log_secret(SSL *ssl, const char *label,
...
int custom_ext_add(SSL *s, int context, WPACKET *pkt, X509 *x,
                          size_t chainidx, int maxversion);

int custom_exts_copy(custom_ext_methods *dst,
